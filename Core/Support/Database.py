...
class Controller:
    ...
    def Gui():
        if os.name == "nt":
            inp = input(Font.Color.RED + "[!]" + Font.Color.WHITE +
                        "SORRY BUT DATABASE IS'NT AVIABLE FOR WINDOWS AT THE MOMENT:(\n\nPRESS ENTER TO CONTINUE")
        else:
            ...
            if mode == "True":
                ...
                os.system("sudo php -S" + host +
                          ":200 -t GUI >/dev/null 2>&1 &")
                sleep(3)
                print(Font.Color.BLUE + "\n[I]" + Font.Color.WHITE +
                      "DATABASE STARTED ON http://{}:200".format(host))
            else:
                os.system("sudo php -S 127.0.0.1:200 -t GUI >/dev/null 2>&1 &")
                sleep(3)
                print(Font.Color.BLUE + "\n[I]" + Font.Color.WHITE +
                      "DATABASE STARTED ON http://127.0.0.1:200")
            inp = input("\nPRESS ENTER TO STOP ")
