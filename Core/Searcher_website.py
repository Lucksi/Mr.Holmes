...
class Web:
    ...
    @staticmethod
    def google_dork(username,report,number,telephone2):
        ...
        f.close()
        if number == True:
            print(Font.Color.GREEN + "\n[+]" + Font.Color.WHITE + "GENERATING LINK FOR {} PHONE NUMBER {}...".format(username,telephone2))
            phonen = "Site_lists/Websites/phoneNumbers_dork.txt"
            f = open(phonen,"r")
            for sites in f:
                site = sites.rstrip("\n")
                site = site.replace("{}", telephone2)
                print(Font.Color.YELLOW + "[+]" + Font.Color.WHITE + site)
                f = open(report,"a")
                f.write(site + "\n")
                sleep(2)
    ...
    def whois_lookup(username,report):
        ...
        if choice == 1:
            Web.google_dork(username,report,number,telephone2)
        else:
            choice = int(input(
            Font.Color.BLUE + "\n[+]" + Font.Color.WHITE + "WOULD YOU LIKE TO PERFORM A TRACEROUTE SCAN?(1)YES(2)NO\n\n" + Font.Color.GREEN + "[#MR.HOLMES#]" + Font.Color.WHITE + "-->"))
            if choice == 1:
