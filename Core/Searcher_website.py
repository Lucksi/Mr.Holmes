...
class Web:
    ...
    def Reputation(username, report):
        ...
        print(Font.Color.GREEN + "\n[+]" + Font.Color.WHITE +
              "SEARCHING WEBSITE/DOMAIN/IP ON DIFFERENT SITES")
        sc = int(input(
        ...
        if sc == 1:
            ...
            identity = "YOUR PROXY IP IS LOCATED IN: {} ({}) ".format(
            ...
            identity = "None"
        print(Font.Color.GREEN + "\n[+]" + Font.Color.WHITE +
              "YOUR PROXY IP ADDRES IS: {} ".format(http_proxy2))
        if identity != "None":
        ...
        for sites in data:
            for data1 in sites:
                ...
                try:
                    ...
                    print(Font.Color.BLUE + "\n[N]" + Font.Color.WHITE +
                          "CONNECTION-ERROR...TRYNG WITH NO PROXIES")
                    http_proxy = None
                    try:
                        Requests_Search.Search.search(error, report, site1, site2, http_proxy, sites, data1, username,
                                                      subject, successfull, name, successfullName, is_scrapable, ScraperSites, Writable, main)
            ...
            print(Font.Color.GREEN + "\n[+]" + Font.Color.WHITE +
                  "{}: {} FOUNDS ON:".format(subject, username))

            if len(successfull):
                for names in successfull:
                    print(Font.Color.YELLOW + "[v]" + Font.Color.WHITE + names)
                print(Font.Color.RED + "\n[!]" + Font.Color.WHITE +
                      "ATTENTION THIS WEBSITE/DOMAIN {} MAY BE DANGEROUS/HAVE SCAM CONTENT..¯\_(ツ)_/¯".format(username))
            else:
                print(Font.Color.YELLOW + "\n[v]" + Font.Color.WHITE +
                      "GOOD LOOKS LIKE THIS WEBSITE/DOMAIN IS SAFE :)")
                f = open(report, "a")
                f.write("GOOD LOOKS LIKE THIS WEBSITE/DOMAIN IS SAFE :-)\r\n")
                f.close()
            print(Font.Color.BLUE + "\n[I]" + Font.Color.WHITE +
                  "FOR MAJOR INFORMATION CONSULT THESE LINKS...")
            print(Font.Color.YELLOW + "[v]" + Font.Color.WHITE +
                  "https://www.scamadviser.com/check-website/{}".format(username))
            print(Font.Color.YELLOW + "[v]" + Font.Color.WHITE +
                  "https://www.ssltrust.com.au/ssl-tools/website-security-check?domain={}".format(username))
            print(Font.Color.YELLOW + "[v]" + Font.Color.WHITE +
                  "https://www.islegitsite.com/check/{}".format(username))
            f = open(report, "a")
            f.write("FOR MAJOR INFORMATION CONSULT THESE LINKS...\n")
            f.write("https://www.scamadviser.com/check-website/{}\n".format(username))
            f.write(
                "https://www.ssltrust.com.au/ssl-tools/website-security-check?domain={}\n".format(username))
        ...
        for sites in final:
            try:
                ...
                if dork.status_code == 200:
                    ...
                    print(Font.Color.YELLOW +
                          "[+]" + Font.Color.WHITE + "ROBOTS SAVED ON: " + robot)
                else:
                    print(Font.Color.RED + "[!]" + Font.Color.WHITE +
                          "OPS UNABLE TO DOWNLOAD {} ROBOTS.TXT..SKIPPING".format(username))
            except Exception as e:
                print(Font.Color.BLUE + "\n[N]" + Font.Color.WHITE +
                      "CONNECTION-ERROR...TRYNG WITH NO PROXIES")
                url = sites["Robots"]["url"].replace("{}", username)
                ...
                if dork.status_code == 200:
                    open(robot, 'wb').write(dork.content)
                    print(Font.Color.YELLOW +
                          "[v]" + Font.Color.WHITE + "ROBOTS SAVED ON: " + robot)
                else:
                    print(Font.Color.RED + "[!]" + Font.Color.WHITE +
                          "OPS UNABLE TO DOWNLOAD {} ROBOTS.TXT..SKIPPING".format(username))
    ...
    def google_dork(username, number, num):
        ...
        if os.path.isfile(report):
            os.remove(report)
            print(Font.Color.BLUE + "\n[I]" + Font.Color.WHITE +
                  "REMOVING OLD {}_Dorks.txt".format(username))
        else:
            pass
        ...
        if number == True:
            ...
            f.close()
            print(Font.Color.GREEN + "\n[+]" + Font.Color.WHITE +
                  "GENERATING LINK FOR {} PHONE NUMBER {}...".format(username, num))
    ...
    def whois_lookup(username, report):
        ...
        if Key == "None":
            ...
            try:
                print(Font.Color.GREEN + "[+]" +
                      Font.Color.WHITE + "API KEY FOUND..")
                Key2 = str(Key)
                ...
                try:
                    ...
                    f = str(e)
                    print(Font.Color.RED + "\n[!]" + Font.Color.WHITE +
                          "OPS SOMETHING WENT WRONG CANNOT ACQUIRE GEODATA SKIPPING...")
                num = telephone2
                if num != "":
                    number = True
                    sc = int(input(Font.Color.BLUE + "\n[?]" + Font.Color.WHITE + "PHONE NUMBER FOUND:{} WOULD YOU LIKE TO EXECUTE A SCAN(1)YES(2)NO".format(
                    ...
                    print(Font.Color.RED +
                          "[!]" + Font.Color.WHITE + "PHONE NUMBER NOT FOUND")
                    number = False
            except Exception as e:
                num = None
                number = False
                print(Font.Color.RED + "[!]" + Font.Color.WHITE +
                      "OPS LOOKS LIKE SOME OF THE DETAILS ARE NOT AVAIABLE DOING CLASSICAL WHO IS...")
                command = ("whois " + username)
                proces = os.popen(command)
                results = str(proces.read())
                final = results + command
                ...
                f.close()
        choice = int(input(
            Font.Color.BLUE + "\n[?]" + Font.Color.WHITE + "WOULD YOU LIKE TO PERFORM A GOOGLE/YANDEX DORK ATTACK?(1)YES(2)NO\n\n" + Font.Color.GREEN + "[#MR.HOLMES#]" + Font.Color.WHITE + "-->"))
        if choice == 1:
    ...
    def search(username):
        ...
        if os.path.isfile(report):
            ...
            print(Font.Color.BLUE + "[I]" + Font.Color.WHITE +
                  "REMOVING OLD {}.txt".format(username))
        print(Font.Color.GREEN + "\n[+]" + Font.Color.WHITE +
              "SEARCH INFORMATION FOR: {}".format(username))
        sleep(3)
        ...
        if status != "fail":
            ...
            print(Font.Color.RED + "[!]" + Font.Color.WHITE +
                  "OPS LOOKS LIKE SERVER DOES NOT RESPONDING SKIPPING...¯\(°_o)/¯ ")
        choice = int(input(
        ...
        if choice == 1:
            ...
            number = False
            choice = int(input(
                Font.Color.BLUE + "\n[?]" + Font.Color.WHITE + "WOULD YOU LIKE TO PERFORM A GOOGLE/YANDEX DORK SCAN?(1)YES(2)NO\n\n" + Font.Color.GREEN + "[#MR.HOLMES#]" + Font.Color.WHITE + "-->"))
            if choice == 1:
                Web.google_dork(username, number, num)
